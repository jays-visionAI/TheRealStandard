...
import { AlertTriangleIcon, ClipboardListIcon, FactoryIcon, DashboardIcon, CheckCircleIcon, PackageIcon } from '../../components/Icons'
...
export default function StepPO() {
    ...
    const handleNext = () => {
        if (currentStep === 2 && selectedSuppliers.length === 0) {
            showAlert('선택 오류', '최소 1개 이상의 매입처를 선택해주세요.', true)
            return
        }
        if (currentStep < 4) {
    ...
    const handleComplete = () => {
        const poCount = selectedSuppliers.length
        showAlert('발주 완료', `✅ 총 ${poCount}개의 매입처로 발주서가 생성되었습니다!\n\n관련 매입처 담당자에게 알림이 전송됩니다.`)
        setTimeout(() => {
